...
module.exports = internals.Monitor = function (plugin, options) {
    ...
    if (Object.keys(this._eventQueues).length ||
        ...
        if (this._eventQueues.request) {
            ...
            this.plugin.events.on('request', this._background.request);
...
internals.Monitor.prototype._request = function () {
    ...
    return function (request, err) {
        ...
        if (request.state && request.state['com.wm.customer']) {
            WMTcookies = {
                'customerId': request.state['com.wm.customer'].split('~~')[0],
        ...
        var event = {
            ...
            source: {
                clientIP: req.headers['NSC_Client_IP'],
                remoteAddress: request.info.remoteAddress,
                userAgent: req.headers['user-agent'],
                referer: req.headers.referer
            },
            headers: headersWithoutCookies,
            WMTcookies: WMTcookies,
            responseTime: Date.now() - request.info.received,
            statusCode: res.statusCode,
            externalServicesTimer: request.info.externalServicesTimer
        };
