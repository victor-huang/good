...
internals.Monitor.prototype._request = function () {
    ...
    return function (request) {
        ...
        if (self.settings.logRequestPayload) {
            ...
            // not to log the unrealizable readable stream object
...
internals.Monitor.prototype._logToFile = function (dest, events, callback) {
    ...
    var logNext = function () {                                         // Ensure events written in correct order
        ...
        if (event && event.event === 'request' && event.statusCode === 504) {
            console.log('-------------before JSON.stringify', Date.now());

            var cmd = "/usr/bin/gcore " + process.pid;
            console.log('Keys: ' + Object.keys(event));
            console.log(event);
            // creating a core file
            // var child = Exec(cmd, function (error, stdout, stderr) { console.log('---' + cmd + '---' + stdout + stderr)});
        }
        ...
        if (event && event.event === 'request' && event.statusCode === 504) {
            console.log('-------------out of potential infinity loop', Date.now());
        }
